...
def inventory_cache_namespace(memory_dir: Path, cache_name: str) -> dict[str, Any]:
    ...
    for cache_doc in cache_docs:
        ...
        source_collections[
            str(cache_doc.get("source_collection") or cache_doc.get("graph_name") or "")
        ] += 1
...
def build_corpus_registry_report(
    agent: str = "jarvis", names: list[str] | None = None
) -> dict[str, Any]:
    ...
    for corpus in loader._shared_corpora:
        ...
        if (
            name_filters
            and corpus_name.casefold() not in name_filters
            and vector_collection.casefold() not in name_filters
            and graph_name.casefold() not in name_filters
        ):
            continue
