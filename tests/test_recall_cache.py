    ...
    projection = project_decomposition(
        {
            ...
            "section_titles": [f"Section {i}" for i in range(90)] + ["Notice of Levy", "Article 9"],
            "entities": [f"Generic Entity {i}" for i in range(260)] + ["United States Postal Service", "Secured Party Creditor", "Uniform Commercial Code", "TreasuryDirect"],
...
def test_infer_projection_profile_supports_explicit_override():
    assert infer_projection_profile(graph_name='notes', requested_profile='legal') == 'legal-retrieval'
    assert infer_projection_profile(graph_name='hammertime-v3', requested_profile='default') == 'default'



def test_infer_projection_profile_supports_reference_and_workflow_profiles():
    assert infer_projection_profile(graph_name='knowledge-base', requested_profile='reference') == 'reference-retrieval'
    assert infer_projection_profile(graph_name='ops-runbooks', requested_profile='workflow') == 'workflow-retrieval'
    assert infer_projection_profile(graph_name='guide-template-corpus', source_ref='docs/reference/tooling.md', payload={'category': 'reference', 'type': 'guide'}) == 'reference-retrieval'
    assert infer_projection_profile(graph_name='ops-corpus', source_ref='runbooks/restart-service.md', payload={'category': 'workflow', 'type': 'process'}) == 'workflow-retrieval'
    ...
    projection = project_decomposition(
        {
            ...
            'section_titles': ['Overview', 'Usage', 'Configuration', 'Troubleshooting'] + [f'Noise {i}' for i in range(80)],
            'entities': [f'Generic Entity {i}' for i in range(160)] + ['OpenAI API', 'Qdrant Client', 'Tool Registry'],
            'claims': [f'generic claim {i}' for i in range(140)] + ['The API supports JSON responses.', 'The endpoint returns structured metadata.', 'The configuration requires an api_key parameter.'],
        },
    ...
    projection = project_decomposition(
        {
            ...
            'section_titles': ['Setup', 'Procedure', 'Verification', 'Checklist'] + [f'Noise {i}' for i in range(80)],
            'entities': [f'Generic Entity {i}' for i in range(160)] + ['Task Queue', 'Agent Router', 'Scheduler Service'],
            'claims': [f'generic claim {i}' for i in range(140)] + ['Run the verification step after restart.', 'Execute the workflow in order.', 'Check the service health before handoff.'],
        },
